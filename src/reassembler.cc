...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring )
{
  ...
  while ( it != unasmb_segments_.end() && it->second.sta_idx <= new_seg.end_idx ) {
    ...
    std::copy( ext_segments.data.begin(), ext_segments.data.end(), merged_data.begin() + ext_offset );

    size_t new_offset = new_seg.sta_idx - merged_start;
    std::copy( new_seg.data.begin(), new_seg.data.end(), merged_data.begin() + new_offset );
